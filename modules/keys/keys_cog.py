...
class KeyLookup(commands.Cog):
    ...
    def save_key_channel(self, data):
        with open(self.KEY_CHANNEL_PATH, "w", encoding="utf-8") as f:
            json.dump(data, f, indent=2)

    @app_commands.command(name="setkeychannel", description="Set this channel for ARC Key info.")
    @app_commands.checks.has_permissions(administrator=True)
    async def set_key_channel(self, interaction: discord.Interaction):
        key_channels = self.load_key_channel()
        guild_id = str(interaction.guild.id)
        key_channels[guild_id] = interaction.channel_id
        self.save_key_channel(key_channels)
        # Send a setup embed in the channel
        embed = discord.Embed(
            title="ARC Key Info Channel Set!",
            description="This channel is now set to receive ARC Key lookups and info.",
            color=0xFFC107
        )
        embed.set_footer(text="You can now use /key in any channel, but this is the designated info channel.")
        await interaction.channel.send(embed=embed)
        await interaction.response.send_message("This channel is now set for ARC Key info.", ephemeral=True)
    ...
    def build_embed(self, key_name: str, data: dict) -> discord.Embed:
        ...
        # Add a thumbnail (provided image)
